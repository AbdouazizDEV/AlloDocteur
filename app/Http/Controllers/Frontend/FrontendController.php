...
class FrontendController extends Controller
{
    ...
    public function doctors(Request $request)
    {
        if($request->has('doctor')){
            $doctors = Doctor::where('designation', 'LIKE', '%'.$request->doctor.'%')->where('is_active', 1)->get();
        }else{
    ...
    {
        $doctor = Doctor::where('slug', $name)->where('is_active', 1)->first();
        if (!$doctor) {
            return redirect()->back()->message("status", "Doctor not found");
        }
